        ...
        private void btnRemoverPaciente_Click(object sender, EventArgs e)
        {
            try
            {
                if (lvwConsultaPaciente.SelectedItems.Count > 0)
                {
                    ...
                    DialogResult resultadoRemoverPaciente = MessageBox.Show("Tem certeza que quer remover o Paciente?", "Tem certeza?", MessageBoxButtons.YesNo);
                    if (resultadoRemoverPaciente == DialogResult.Yes)
                    {
                        foreach (ListViewItem item in lvwConsultaPaciente.SelectedItems)
                        {
                            ...
                            {
                                DialogResult resultadoRemoverConsulta = MessageBox.Show("Remover esse paciente irá remover todas as consultas marcadas. Tem certeza?", "Tem certeza?", MessageBoxButtons.YesNo);
                                if (resultadoRemoverConsulta == DialogResult.Yes)
                                {
                                    List<Consulta> removerConsulta = new List<Consulta>();
                                    foreach (var consulta in pacienteRemover.Consultas)
                                    {
                                        removerConsulta.Add(consulta);
                                    }
                                    foreach (var consulta in removerConsulta)
                                    {
                                        pacienteRemover.Consultas.Remove(consulta);
                                    }
                                    lvwConsultaPaciente.Items.Remove(item);
                                    lvwConsultasMarcadas.Clear();
                                    pacientes.Remove(pacienteRemover);
                                    MessageBox.Show("Paciente removido!", "Sucesso!");
                                    return;
                                }
                            }
                ...
                {
                    MessageBox.Show("Selecione um paciente antes de remover!", "Erro");
                }
            }
            catch (Exception ex)
            {
