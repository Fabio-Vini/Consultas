...
namespace Consultas_Médicas
{
    public partial class frmAdicionarSaldo : Form
    {
        ...
        private void btnRemoverSaldo_Click(object sender, EventArgs e)
        {
            try
            {
                if (lvwConsultaPaciente == null)
                ...
                if (lvwConsultaPaciente.SelectedItems.Count > 0)
                {
                    ...
                    {
                        double valorAdicionar;
                        if (double.TryParse(txtRemoverSaldo.Text, out valorAdicionar))
                        {
                            pacienteSelecionado.DescontarSaldo(valorAdicionar);
                        ...
                        {
                            MessageBox.Show("Por favor, insira um valor válido para remover ao saldo.");
                        }
                    }
                    else
                    {
            ...
            {
                MessageBox.Show("Erro ao remover saldo: " + ex.Message, "Erro", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
    }
}